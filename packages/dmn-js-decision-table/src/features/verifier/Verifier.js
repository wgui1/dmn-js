      ...
      if (firstRule !== undefined) {
        ...
        if (!moreThanTwoColumnsDiffer && differentColumnIndex > -1) {
          ...
          if (currentColumnType === 'string' || currentColumnType === 'boolean') {
            newValue.value = '[' + firstHalf.replace(/[[\]\+]/g, '') +
...
Verifier.prototype.findOverlappingRules = function(comparingRules, index, inputColumnCount) {
  if (inputColumnCount === index) {
    ...
    var isSubset = false;
    var currentRuleHasSubsetInList = false;
    ...
    for (var listIndex = 0; listIndex < listLen; listIndex++) {
      ...
      if (similarCount === Object.keys(currentList).length) {
        currentRuleHasSubsetInList = true;
    ...
    if (!isSubset || currentRuleHasSubsetInList) {
      var keyMap = {};
    ...
    for (var i = 0; i < sortedRuleOrder.length; i++) {
      ...
      if (newIntervalValue && ruleIds.length > 1 && !ruleStart && lastIntervalIsStartInterval) {
        var newRules = {};
...
Verifier.prototype.findMissingRules = function(comparingRules, currentRule, index, inputColumnCount) {
  if (inputColumnCount > index) {
    ...
    if ((ruleType === 'string' || ruleType === 'boolean') && allowedValues.length > 0) {
      ...
      missingRuleList = this.mergeRules(missingRuleList, missingInterval, inputColumnCount);
...
Verifier.prototype.addMissingRule = function(modeling) {
  return function() {
    ...
    for (var i = 0; i < ruleValues.length; i++) {
      if (ruleValues[i] !== 'any') {
        if (inputTypes[i] === 'string' || inputTypes[i] === 'boolean') {
          modeling.editCell(newRowId, inputClauseIDs[i],
            (ruleValues[i].toString()).replace(/[(\[\)\]\+]/g, ''));
        } else {
          modeling.editCell(newRowId, inputClauseIDs[i], (ruleValues[i].toString()));
        }
      }
    }
    ...
    if (value.indexOf('<') > -1) {
      ...
      if (value.indexOf('=') > -1) {
        ...
        if (type === 'integer' || type === 'long') {
          this.equalSigns = [false, true];
          this.intervalValues = [-Infinity, numericValue-1];
      ...
      if (intervalValues[0].indexOf('[') > -1) {
        ...
        equalSigns[0] = true;
        this.intervalValues = [Number(intervalValuesWithoutBrackets[0])+1,
          Number(intervalValuesWithoutBrackets[1])];
        intervalValues[0] = '[' + (Number(intervalValuesWithoutBrackets[0])+1).toString();
      }
      if (intervalValues[1].indexOf(']') > -1) {
        equalSigns[1] = true;
      } else if (type === 'integer' || type === 'long') {
        equalSigns[1] = true;
        this.intervalValues = [Number(intervalValuesWithoutBrackets[0]),
          Number(intervalValuesWithoutBrackets[1])-1];
        intervalValues[1] = ' ' + (Number(intervalValuesWithoutBrackets[1])-1).toString() + ']';
      }
      this.equalSigns = equalSigns;
      this.value = intervalValues[0] + ', ' + intervalValues[1];
    } else if (value === '') {
      this.intervalValues = [-Infinity, Infinity];
      this.equalSigns = [false, false];
    } else {
