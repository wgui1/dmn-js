...
OptimizeTable.prototype.optimizeDMNTable = function() {
  ...
  inputColumnTypes = this.getInputTypes(inputIDs);
  ...
  var elements = [];
  for (var index = 0; index < inputColumnTypes.length; index++) {
    elements.push(index);
  }
  var columnPermutation = this.calculatePermutation([], elements, []);
  ...
  for (var overlapID in outputEntries) {
    ...
    if (Object.keys(rulesWithSameOutput).length > 1) {
      ...
      var ruleGroups = this.findOverlappingRules(rulesWithSameOutput, 0, inputColumnCount, []);
      for (var groupNR in ruleGroups) {
        var currentRules = ruleGroups[groupNR];

        if (Object.keys(currentRules).length > 1) {
          var splittedRules = this.splitTheRules(currentRules, inputColumnCount);


          var smallest = Infinity;
          var smallestObject = {};
          for (var permutationIndex = 0; permutationIndex < columnPermutation.length; permutationIndex++) {
            var splittedRulesCopy = (JSON.parse(JSON.stringify(splittedRules)));
            for (var idd in splittedRulesCopy) {
              var ruleee = splittedRulesCopy[idd];
              for (var rrr = 0; rrr < ruleee.length; rrr++) {
                if (ruleee[rrr].type === 'double') {
                  if (ruleee[rrr].intervalValues[0] === null) {
                    ruleee[rrr].intervalValues[0] = -Infinity;
                  }
                  if (ruleee[rrr].intervalValues[1] === null) {
                    ruleee[rrr].intervalValues[1] = Infinity;
                  }
                }
              }
            }

            var mergedRules = this.hyperplaneSweep(splittedRulesCopy, columnPermutation[permutationIndex]);
            if (Object.keys(mergedRules).length < smallest) {
              smallest = Object.keys(mergedRules).length;
              smallestObject = mergedRules;
            }
          }


          for (var mergeRuleID in smallestObject) {
            currentRuleList[mergeRuleID] = smallestObject[mergeRuleID];
          }
        } else {
          var ruleID = Object.keys(currentRules)[0];
          currentRuleList[ruleID] = currentRules[ruleID];
        }

      }
      newRules.push(currentRuleList);
    } else {
...
OptimizeTable.prototype.calculatePermutation = function(permutation, elementList, permList) {
  if (elementList.length === 0) {
    permList.push(permutation);
  } else {
    for (var index = 0; index < elementList.length; index++) {
      var permutationCopy = permutation.slice(0);
      permutationCopy.push(elementList[index]);
      var elementListCopy = elementList.slice(0);
      elementListCopy.splice(index, 1);
      this.calculatePermutation(permutationCopy, elementListCopy, permList);
    }
  }
  return permList;
...
OptimizeTable.prototype.findOverlappingRules = function(comparingRules, index, inputColumnCount, rules) {
  if (inputColumnCount === index) {
    rules.push(comparingRules);
  } else {
    var stringIndex = 0;
    var currentRuleStringValues = '';
    var currentStringValue = '';
    var overlappingRules = {};
    var currentColumnType = inputColumnTypes[index];
    if (currentColumnType === 'string' || currentColumnType === 'boolean') {
      var firstRuleId = Object.keys(comparingRules)[0];
      var allValues = comparingRules[firstRuleId][index].allowedValues.slice(0);
      var ruleId = NaN;
      if (allValues.length === 1) {
        this.findOverlappingRules(comparingRules, index + 1, inputColumnCount, rules);
      } else {
          ...
          if (StringValue !== '') {
            if (Object.keys(overlappingRules).length === 0) {
              for (ruleId in comparingRules) {
                ...
                currentRuleStringValues = rule.multipleStringValues;
                if (rule.multipleStringValues.indexOf(StringValue) > -1) {
                  for (stringIndex = 0; stringIndex < currentRuleStringValues.length; stringIndex++) {
                    currentStringValue = currentRuleStringValues[stringIndex];
                    overlappingRules[currentStringValue] = listSize;
                  }
                }
              }
            } else {
              var currentRules = [];
              var overlapWithPreviousRules = false;
              var ruleGroupNR = -1;
              var newRules = [];
              var twoGroupOverlap = false;
              for (ruleId in comparingRules) {
                rule = comparingRules[ruleId][index];
                currentRuleStringValues = rule.multipleStringValues;
                if (rule.multipleStringValues.indexOf('') > -1) {
                  hasWildCard = true;
                  this.findOverlappingRules(comparingRules, index + 1, inputColumnCount, rules);
                  break;
                } else {
                  if (rule.multipleStringValues.indexOf(StringValue) > -1) {
                    for (stringIndex=0; stringIndex < currentRuleStringValues.length;stringIndex++) {
                      currentStringValue = currentRuleStringValues[stringIndex];
                      if (overlappingRules[currentStringValue] !== undefined) {
                        currentRules.push(currentStringValue);
                        overlapWithPreviousRules = true;
                        if (ruleGroupNR === -1) {
                          ruleGroupNR = overlappingRules[currentStringValue];
                        } else if (ruleGroupNR !== overlappingRules[currentStringValue]) {
                          twoGroupOverlap = true;
                          if (overlappingRules[currentStringValue] < ruleGroupNR) {
                            ruleGroupNR = overlappingRules[currentStringValue];
                          }
                        }
                      } else {
                        newRules.push(currentStringValue);
                      }
                    }
                    if (twoGroupOverlap) {
                      for (var id in currentRules) {
                        overlappingRules[currentRules[id]] = ruleGroupNR;
                      }
                    }
                  }
                }
              }
              if (!overlapWithPreviousRules) {
                listSize++;
                ruleGroupNR = listSize;
              }
              if (!hasWildCard) {
                for (ruleId in newRules) {
                  overlappingRules[newRules[ruleId]] = ruleGroupNR;
                }
              }
            }
            if (hasWildCard) {
        ...
        if (!hasWildCard) {
          var rulesThatOverlap = {};
          for (ruleId in comparingRules) {
            rule = comparingRules[ruleId][index];
            currentRuleStringValues = rule.multipleStringValues[0];
            var groupNR = overlappingRules[currentRuleStringValues];
            if (rulesThatOverlap[groupNR] === undefined) {
              rulesThatOverlap[groupNR] = {};
              rulesThatOverlap[groupNR][ruleId] = comparingRules[ruleId];
            } else {
              rulesThatOverlap[groupNR][ruleId] = comparingRules[ruleId];
            }

          }
          for (var groupID in rulesThatOverlap) {
            this.findOverlappingRules(rulesThatOverlap[groupID], index + 1, inputColumnCount, rules);
          }
        }
      }
    } else {
      overlappingRules = {};
      var rulesForNextDimension = {};
      ...
      for (var i = 0; i < sortedRuleOrder.length; i++) {
        var currentRuleID = Object.keys(sortedRuleOrder[i])[0];
        var rule = sortedRuleOrder[i][currentRuleID];
        var ruleValue = rule.value;
        var ruleStart = rule.start;
        var ruleEqual = rule.equal;
        var valuesMeet = false;

        if (rule.start && ((currentColumnType === 'integer' || currentColumnType === 'long') &&
        parseInt(lastIntervalValue) + 1 === parseInt(ruleValue) && lastIntervalEqualSign && rule.equal) ||
        (currentColumnType === 'double' && (lastIntervalValue === ruleValue) &&
        ((lastIntervalEqualSign && !rule.equal) || (!lastIntervalEqualSign && rule.equal)))) {
          valuesMeet = true;
        }
        if ((Object.keys(overlappingRules).length === 0 && i !== 0 && !valuesMeet) ||
        i === sortedRuleOrder.length-1) {
          this.findOverlappingRules(rulesForNextDimension, index + 1, inputColumnCount, rules);
          rulesForNextDimension = {};
        }
        if (ruleStart) {
          overlappingRules[currentRuleID] = rule;
          rulesForNextDimension[currentRuleID] = comparingRules[currentRuleID];
        } else {
          delete overlappingRules[currentRuleID];
        }
...
OptimizeTable.prototype.hyperplaneSweep = function(comparingRules, dimensionOrder) {

  var columnCount = inputColumnTypes.length;
  for (var index = 0; index < columnCount; index++) {
    // var columnIndex = dimensionOrder[index].column;
    var columnIndex = dimensionOrder[index];
    ...
    if (ruleType === 'string' || ruleType === 'boolean') {
      ...
      for (var i = 0; i < sortedRuleOrder.length; i++) {
        ...
        if (comparingRules[ruleId] !== undefined) {
          var rule = sortedRuleOrder[i][ruleId];
          var ruleValue = rule.value;
          if (!rule.start && Object.keys(meetsList).length === 0) {
            backList[ruleId] = comparingRules[ruleId];
          } else if (rulesDoNotMeet && rule.start && (
          ...
          lastIntervalEqualSign === rule.equal) {
            meetsList[ruleId] = comparingRules[ruleId];
          } else if (rulesDoNotMeet || (Object.keys(meetsList).length === 0 && rule.start &&
          lastIntervalValue === ruleValue && lastIntervalEqualSign === rule.equal)) {

          } else {
            comparingRules = this.mergeRules(backList, meetsList,
...
OptimizeTable.prototype.calculateDimensionOrder = function(splittedRules, columnCount) {
...
OptimizeTable.prototype.mergeRulesCategoryValues = function(columnIndex, columnCount, comparingRules) {
...
OptimizeTable.prototype.mergeRules = function(backList, meetsList,
    columnIndex, columnCount, comparingRules) {
  var newBackList = {};
  var col = 0;
  var colType = '';
  var firstHalf = '';
  ...
  var newRules = this.mergeAdjacencyRules(newBackList, comparingRules, columnCount);
  // var newRules = comparingRules;

  return newRules;
};


OptimizeTable.prototype.mergeAdjacencyRules = function(newBackList, comparingRules, columnCount) {
...
OptimizeTable.prototype.mergeRulesBaseline = function(splittedRules, columnCount) {
  var mergeHappened = true;
  while (mergeHappened) {
    mergeHappened = false;
    var ruleIDs = Object.keys(splittedRules);
    var ruleCount = ruleIDs.length;

    for (var firstRuleIndex = 0; firstRuleIndex < ruleCount; firstRuleIndex++) {
      var firstRule = splittedRules[ruleIDs[firstRuleIndex]];
      if (firstRule !== undefined) {
        for (var secondRuleIndex = firstRuleIndex+1; secondRuleIndex < ruleCount; secondRuleIndex++) {
          var secondRule = splittedRules[ruleIDs[secondRuleIndex]];
          var differentColumnIndex = -1;
          var moreThanTwoColumnsDiffer = false;
          for (var col = 0; col < columnCount; col++) {
            if (firstRule[col].value !== secondRule[col].value) {
          ...
          if (!moreThanTwoColumnsDiffer) {
            ...
            if (colType === 'string' || colType === 'boolean') {
              ...
              var newCharValue = oldValue + ', ' + secondRule[differentColumnIndex].value;
              newValue.allowedValues = cellValues.allowedValues;
              var stringOldValues = firstRule[differentColumnIndex].multipleStringValues;
              var stringValues = secondRule[differentColumnIndex].multipleStringValues;
              for (var stringIndex = 0; stringIndex < stringValues.length; stringIndex++) {
                if (stringOldValues.indexOf(stringValues[stringIndex]) === -1) {
                  stringOldValues.push(stringValues[stringIndex]);
                }
            ...
            if (newValue !== 'Impossible to merge') {
              ...
              newValue.type = colType;
              splittedRules[ruleIDs[secondRuleIndex]][differentColumnIndex] = newValue;
              delete splittedRules[ruleIDs[firstRuleIndex]];
              mergeHappened = true;
              break;
            }
          }
...
OptimizeTable.prototype.splitTheRules = function(comparingRules, inputColumnCount) {
